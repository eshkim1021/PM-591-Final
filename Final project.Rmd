...
refine_data$AGE_NEONATE <- factor(refine_data$AGE_NEONATE,
                                  levels = c(0,1),
                                  labels = c("Non-neonatal Age","Neonatal Age"))
...
#make log learner 
data_learn_log <- makeLearner("classif.", predict.type = "prob",
                              fix.factors.prediction = TRUE)

holdout_desc <- makeResampleDesc(method = "Holdout", stratify = TRUE)

log_split <- makeResampleInstance(holdout_desc,log_tsk, split = 0.7)
...
data_rf <- randomForest(DIED ~., data = refine_data[rf_train, ],)
```
