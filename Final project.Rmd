...
analyzeFeatSelResult(log_forward, reduce = TRUE)

forward_log_data <- refine_data %>% select("DIED","APRDRG_Risk_Mortality","RACE","LOS")

log_for_tsk <- makeClassifTask(id = "Paitent Mortality", data = forward_log_data,
                               target = "DIED")

crossval(data_learn_log,log_for_tsk,iters = 10L, stratify = TRUE, measures = auc)
```

```{r}

data_train_log <- train(data_learn_log, data_tsk,subset = log_train)

log_predict <- predict(data_train_log,task = data_tsk,subset = log_test)

log_predict

performance(log_predict,measures = list(mmce,acc))
```
